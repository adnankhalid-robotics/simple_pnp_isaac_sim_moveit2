...
#include "isaac_moveit_package/srv/cube_pose.hpp"
...
using GetCubePose = isaac_moveit_package::srv::CubePose;
...
int main(int argc, char** argv)
{
  ...
  // SERVICE CLIENT
  auto client = node->create_client<GetCubePose>("/cube_pose");
  ...
  // ORIENTATION FIX : align the FINGERS to the cube faces.
  // 45 deg compensation for the panda_link8 -> panda_hand mounting offset.
  const double GRIPPER_YAW_OFFSET = M_PI / 4.0;

  // Cube yaw (rotation about the base Z axis) from the service quaternion.
  double cube_yaw = std::atan2(
      2.0 * (response->qw * response->qz + response->qx * response->qy),
      1.0 - 2.0 * (response->qy * response->qy + response->qz * response->qz));

  // Snap into [-45 deg, +45 deg] using the cube's 4-fold (90 deg) symmetry.
  ...
  RCLCPP_INFO(LOGGER,
              "Cube yaw: %.3f rad | offset: %.3f rad -> grasp yaw (after symmetry): %.3f rad",
              cube_yaw, GRIPPER_YAW_OFFSET, grasp_yaw);
